  ...
  changeColumnQuery(tableName, attributes) {
    ...
    for (const attributeName in attributes) {
      ...
      if (definition.match(/REFERENCES/)) {
        const fkName = this.quoteIdentifier(tableName + '_' + attributeName.replace(/_/g,' ').match(/\b(\w{1,2})/g).join('') + '_foreign_idx');
  ...
  attributeToSQL(attribute, options) {
    ...
    if (attribute.references) {
      ...
      if (options && options.context === 'addColumn' && options.foreignKey) {
        ...
        const idxMiddle = attrName.replace(/_/g,' ').match(/\b(\w{1,2})/g).join('');
        const fkName = this.quoteIdentifier(`${options.tableName}_${idxMiddle}_foreign_idx`);
