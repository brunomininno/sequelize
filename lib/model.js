          ...
          return Promise.all([
            ...
            .then(tableInfos => {
              ...
              _.each(attributes, (columnDesc, columnName) => {
                let currentAttributes = attributes[columnName]
                if (!columns[columnDesc.field]) {
                  changes.push(() => this.QueryInterface.addColumn(this.getTableName(options), currentAttributes.field, currentAttributes));
                }
              });
              _.each(columns, (columnDesc, columnName) => {
                let currentAttributes = null;
                _.each(attributes, (aDesc, aName) => {
                  if (aDesc.field === columnName) {
                    currentAttributes = aDesc
                  }
                })
